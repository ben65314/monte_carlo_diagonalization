...
colors = ['black','red','green','orange','blue','yellow','cyan','lime',
          'orange','blue','black','red','green','orange','blue']
    ...
    with open(data_file_name) as file:
        ...
        for lines in reader:
            ...
            if (lines[0].startswith('NEXT_SITE')):
                if is_both_data : 
                    plt.plot(data[0], data[1], lw=1, ls='dashed', 
                             c=colors[i%15])
                    plt.plot(data[0], data[2], lw=1, ls='dotted', 
                             c=colors[i%15])
...
title = ("#Sites = " + str(param[0]) + "\tU = " + str(param[1]) + "\t$\mu$ = "
        + str(param[2]) + "\tN = " + str(param[3]) + "\t$S_z$ = " 
        + str(param[4]))
plt.title(title)
